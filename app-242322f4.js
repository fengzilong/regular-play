...
/***/ function(module, exports) {
	...
	exports.default = {
		...
		config: function config() {
			...
			this.data.nextLayout = getNext(this.layout, this.layouts);
		},
		onChangeLayout: function onChangeLayout() {
			...
			Object.keys(oldParams).forEach(function (k) {
				oldParams[k] = decode(oldParams[k]);
			});
			...
			var params = router.encode('app', Object.assign({}, router.current.param, {
				layout: this.data.nextLayout
			}));
			console.log('params', params);
			router.go('app', params);

			setTimeout(function () {
				_this.data.nextLayout = getNext(_this.layout, _this.layouts);
				_this.$update();
			}, 10);
		}
	};
