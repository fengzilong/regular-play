...
/***/ function(module, exports) {
	...
	exports.default = {
		...
		template: '\n\t\t<button on-click="{ this.dispatch( \'changeLayout\', \'mobile\' ) }">{ nextLayout }</button>\n\t',
		config: function config() {
			var layouts = this.$get('layouts');
			var layout = this.$get('layout');
			this.nextLayout = getNextLayout(layout, layouts);
		}
	};


	function getNextLayout(layout, layouts) {
		var index = layouts.indexOf(layout) || 0;
	}
