...
/***/ function(module, exports) {
	...
	exports.default = {
		...
		config: function config() {
			var layouts = this.data.layout = 'desktop';
			var layout = this.data.layouts = ['desktop', 'mobile'];
			this.data.nextLayout = getNext(layout, layouts);
		},
		onChangeLayout: function onChangeLayout() {
			var router = this.$router;
			var newParam = router.encode(Object.assign({}, router.current.param, {
				layout: this.data.nextLayout
			}));
			router.go('app', newParam);
		}
	};
